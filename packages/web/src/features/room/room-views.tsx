...
import type {
  ...
  MovieCandidate,
  SwipeChoice,
} from "@deckflix/shared/game-core";
import type {
  DisplayGameState,
  GameMeta,
  GamePlayers,
...
export function DisplayRoomView({
  ...
  const metaQuery = useQuery<GameMeta>({
    ...
    queryFn: () =>
      scopedToActiveRoom
        ? parseRpc(api.api.rooms.me.meta.$get())
        : parseRpc(
          api.api.rooms[":gameCode"].meta.$get({
            param: {gameCode: normalizeGameCode(gameCode)},
          }),
  ...
  const playersQuery = useQuery<GamePlayers>({
    ...
    queryFn: () =>
      scopedToActiveRoom
        ? parseRpc(api.api.rooms.me.players.$get())
        : parseRpc(
          api.api.rooms[":gameCode"].players.$get({
            param: {gameCode: normalizeGameCode(gameCode)},
          }),
  ...
  const stateQuery = useQuery<DisplayGameState>({
    ...
    queryFn: () =>
      scopedToActiveRoom
        ? parseRpc(api.api.rooms.me.display.state.$get())
        : parseRpc(
          api.api.rooms[":gameCode"].display.state.$get({
            param: {gameCode: normalizeGameCode(gameCode)},
          }),
        ),
  });
  ...
  const deleteRoomMutation = useMutation({
    mutationFn: async () =>
      scopedToActiveRoom
        ? parseRpc(api.api.rooms.me.$delete())
        : parseRpc(
          api.api.rooms[":gameCode"].$delete({
            param: {gameCode: normalizeGameCode(gameCode)},
          }),
        ),
    onSuccess: () => {
...
export function PlayerRoomView({
  ...
  const resultsQuery = useQuery<GameResults>({
    ...
    queryFn: () =>
      scopedToActiveRoom
        ? parseRpc(api.api.rooms.me.results.$get())
        : parseRpc(
          api.api.rooms[":gameCode"].results.$get({
            param: {gameCode: normalizeGameCode(gameCode)},
          }),
        ),
  });
  ...
  const stateQuery = useQuery<PlayerGameState>({
    ...
    queryFn: () =>
      scopedToActiveRoom
        ? parseRpc(api.api.rooms.me.player.state.$get())
        : parseRpc(
          api.api.rooms[":gameCode"].players.me.$get({
            param: {gameCode: normalizeGameCode(gameCode)},
          }),
        ),
  });
  ...
  const voteMutation = useMutation({
    mutationFn: async (payload: {
      ...
      scopedToActiveRoom
        ? parseRpc(
          api.api.rooms.me.player.votes.$post({
            json: {
        ...
        : parseRpc(
          api.api.rooms[":gameCode"].players[":playerId"].votes.$post({
            param: {
  ...
  const leaveMutation = useMutation({
    mutationFn: async (playerId: string) =>
      scopedToActiveRoom
        ? parseRpc(api.api.rooms.me.player.leave.$post())
        : parseRpc(
          api.api.rooms[":gameCode"].players[":playerId"].leave.$post({
            param: {
              gameCode: normalizeGameCode(gameCode),
              playerId,
            },
          ...
          <div className="text-center text-muted-foreground">
            {state.summary.playerCount < 2 ? (
              ...
              <>
                ...
                <p className="mt-1 text-sm">
                  Watch the display for the final board.
                </p>
              </>
            )}
          </div>
